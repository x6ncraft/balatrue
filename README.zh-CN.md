...
游戏已经形成经过测试的纯前端发布候选。项目原创代码采用 MIT 许可证，已经具备公开源码的许可
基础。现有 Git 历史曾包含完整上游效果与解锁原文，因此切换仓库可见性前，还要在用户确认后建立
干净根提交或新的公开仓库。目前没有部署公开生产站点，也不声称取得 Balatro 权利人的授权。
...
- **输入联想：**支持游戏内英文名、简体中文名、全拼和拼音首字母；所有匹配项都保留在可滚动
  列表中。
- **线索字典：**随时查看全部玩家线索，不影响战绩。
- **小丑图鉴：**可搜索、筛选 150 张牌；每日局进行中打开后会标记为图鉴辅助局，不计战绩。
- **本地战绩与分享：**记录胜率、连胜和平均出牌数，并生成不泄露答案的分享结果。
- **移动端：**常见手机宽度下，牌面身份与五项线索保持六列对齐；340px 及以下使用更易读的
  兜底布局。
...
可部署产物位于 `dist/`。干净 checkout 无需本地来源审计文件，也能完成安装、校验、测试与构建。
远程数据同步只供维护者在确认数据源访问条件后使用，默认关闭；执行前先读
...
data/         随仓库提交的逐卡来源记录；本地受限审计数据已被 Git 忽略
scripts/      数据生成、数据校验与发布产物检查
tests/e2e/    Playwright 浏览器旅程
docs/         产品、架构、设计、权利、发布与部署记录
...
浏览器目录只包含玩法需要的名称与事实、项目整理的分类和来源摘要。完整英文效果与解锁原文只
保存在维护者本地、被 Git 忽略的审计文件中；干净 checkout 和生产构建都不依赖、也不包含它。

发布或再分发前，请先读[素材与数据说明](ASSET_NOTICE.md)、
