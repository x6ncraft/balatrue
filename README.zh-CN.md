...
小丑牌，系统会从稀有度、基础价格或获取方式、主效果、触发时机和判定依据五个方面给出线索。
...
- **五项线索：**稀有度、基础价格或获取方式、主效果、触发时机和判定依据。
- **分层线索：**主效果、触发时机和判定依据各使用 7 个大类；主效果与判定依据展示猜测牌自身的
  完整细项，触发时机只保留 7 个直观阶段。绿、黄、灰负责表达完全、部分、不吻合，不逐项指出
  具体哪里吻合，也不透露谜底独有的内容。
- **输入联想：**支持游戏内英文名、简体中文名、全拼和拼音首字母；所有匹配项都保留在可滚动
  列表中。
- **线索字典：**随时查看比较类别、主效果机制、动态行为和判定细项，不影响战绩；五个列头都能
  直达对应章节。
- **小丑图鉴：**可搜索、筛选 150 张牌，并保留完整分类说明；每日局进行中
  打开后会标记为图鉴辅助局，不计战绩。
- **本地战绩与分享：**记录胜率、连胜和平均出牌数，并生成不泄露答案的分享结果。
- **移动端：**低至 320px 宽度，牌面身份与五项线索仍保持六列对齐；棋盘与字典始终使用同一套
  分类名称，完整细项留在所属格内并自然换行，仅缩短价格列头以留出阅读空间。
...
- [线索分类与回归制度](docs/clue-taxonomy.md)
