...
更新日期：2026-07-19。本文件记录项目采用的权利边界与发布判断，不构成法律意见。
...
页面短说明由 `product.md` 和 `src/i18n/messages.ts` 共同维护，准确表达“粉丝制作、免费、非商业、
非官方”；完整说明同时写清无关联、无背书、权利归属和权利／下架联系。以后加入广告、赞助、
捐赠、付费或奖品时，必须重新核对权利边界和页面文案。
...
1. **项目原创内容：**根 `LICENSE` 只以 MIT 许可项目原创的功能定义、游戏逻辑、UI 与工程代码，
   以及承载这些原创内容的项目文档。`package.json` 的 `private: true` 只防止误发 npm，不限制
   GitHub 公开。MIT 不覆盖第三方名称、商标、游戏文本、卡图、字体或依赖。
...
4. **本地审计原文：**完整英文效果与解锁文案只保存在 `data/restricted/*.json` 的维护者本地副本。
   该路径已被 Git 忽略，不进入普通提交或生产构建；干净 checkout 不依赖它。

素材与数据的短版声明见 `ASSET_NOTICE.md`，软件和字体声明见 `THIRD_PARTY_NOTICES.md`。

## 现有 Git 历史

当前文件树已经移除完整英文效果与解锁原文的公开依赖，但现有 Git 历史曾在生成目录中提交过这些
原文。直接把现有仓库切换为 public 会同时公开历史版本，违背当前的分发边界。

正式公开源码前，应从当前文件树建立干净根提交，或创建新的公开仓库，再从干净 checkout 完成
安装、校验、测试与构建。当前发布口径允许公开 MIT 原创代码、公开运行目录、逐卡来源记录和明确
排除 MIT 的低分辨率卡图；不再以替换卡图作为公开源码的必选前置。创建新仓库、改写历史、切换
可见性和推送新根提交都属于外部或破坏性动作，必须等待用户明确确认。
...
当前页面使用公开的 [@x6ncraft](https://x.com/x6ncraft) 作为权利咨询与下架入口。注册生产域名后，
应建立 `legal@所选域名` 或同等稳定的邮箱，并同步更新页面、README 和本文件。
