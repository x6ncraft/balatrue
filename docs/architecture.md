...
src/data/jokers.generated.ts
  → game/clue-model.ts  c11 精确分类投影为 g9 玩家线索
...
- `clue-model.ts` 把 31 个审计主效果机制投影为 7 个棋盘大类，把 22 个精确时机投影为 8 个比较
  大类，并把 16 个来源条件族投影为 7 个玩家大类；主效果机制、精确时机与条件细项继续保留。
- `compare.ts` 只比较 `Joker` 的版本化公开玩法字段：主效果完整机制、时机精确事件或条件完整细项
  集合相同为绿；集合不同但玩家大类有交集为黄；大类无交集为灰。
- `visible-feedback.ts` 只按玩家实际看见的大类、猜测牌细项、颜色和方向估算剩余候选；不可见的
  逐项比较信息不得暗中缩小范围。
- `persistence.ts` 把数据分类版本与玩家线索版本写入键名；从 `g8`、`g7`、`g6` 等历史键恢复时，
  由调用方按 `g9` 依次重放猜牌并重算反馈，只有答案、猜牌顺序、胜负和辅助状态均保持一致才写入
  当前键。
